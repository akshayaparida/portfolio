...
**Homogeneous Equation (Lines passing through origin):**
ax² + 2hxy + by² = 0

Represents two lines y = m₁x and y = m₂x where:
- m₁ + m₂ = -2h/b
- m₁m₂ = a/b

**Angle between lines (θ):**
tan θ = |(2√(h² - ab)) / (a + b)|
- Parallel/Coincident if h² = ab
- Perpendicular if a + b = 0

**General Equation (Lines not necessarily through origin):**
ax² + 2hxy + by² + 2gx + 2fy + c = 0
Represents a pair of straight lines if determinant Δ = 0:
Δ = abc + 2fgh - af² - bg² - ch² = 0

\`\`\`python path=null start=null
import numpy as np

def analyze_pair_of_lines(a, h, b):
    """Analyze ax² + 2hxy + by² = 0"""
    # Slopes m = (-h ± √(h²-ab))/b
    discriminant = h**2 - a*b
    
    if discriminant < 0:
        return "Imaginary lines (Point at origin)"
    elif discriminant == 0:
        return "Coincident lines"
    
    term = np.sqrt(discriminant)
    tan_theta = (2 * term) / abs(a + b) if (a + b) != 0 else float('inf')
    angle_deg = np.degrees(np.arctan(tan_theta))
    
    return f"Real lines, Angle: {angle_deg:.2f}°"

# Example: x² - 5xy + 6y² = 0
# Compare to ax² + 2hxy + by² = 0
# a=1, 2h=-5 -> h=-2.5, b=6
print(analyze_pair_of_lines(1, -2.5, 6))
\`\`\`
